// 以下文件来自 https://github.com/wong2/weixin-agent-sdk (packages/sdk/src/media/mime.ts)
...
/** 根据文件扩展名获取 MIME 类型；未知扩展名返回 `application/octet-stream`。 */
export function getMimeFromFilename(filename: string): string {
  const ext = path.extname(filename).toLowerCase();
  return EXTENSION_TO_MIME[ext] ?? "application/octet-stream";
}

/** 根据 MIME 类型获取文件扩展名；未知类型返回 `.bin`。 */
export function getExtensionFromMime(mimeType: string): string {
  const ct = mimeType.split(";")[0].trim().toLowerCase();
  return MIME_TO_EXTENSION[ct] ?? ".bin";
}

/** 根据 Content-Type 头或 URL 路径推断扩展名；未知时返回 `.bin`。 */

// 以下文件来自 https://github.com/wong2/weixin-agent-sdk (packages/sdk/src/monitor/monitor.ts)
...
function extractCommandText(itemList?: MessageItem[]): string {
...
function isImmediateControlCommand(full: WeixinMessage): boolean {
  const text = extractCommandText(full.item_list);
  return /^\/(?:do|undo)(?:\s|$)/i.test(text);
}
...
/**
 * 长轮询主循环：getUpdates -> 处理消息 -> 调用 agent -> 发送回复。
 * 会一直运行到收到 abort 信号为止。
 */
export async function monitorWeixinProvider(opts: MonitorWeixinOpts): Promise<void> {
  ...
  const conversationTasks = new Map<string, Promise<void>>();
    ...
    try {
      ...
      if (isApiError) {
        ...
        if (isSessionExpired) {
          ...
          errLog(
            `[weixin] session expired (errcode ${SESSION_EXPIRED_ERRCODE}), pausing for ${Math.ceil(pauseMs / 60_000)} min. Please run \`npx weixin-acp-router login\` to re-login.`,
          );
      ...
      for (const full of list) {
        ...
        if (isImmediateControlCommand(full)) {
          void run().catch((error) => {
            errLog(`[weixin] immediate control command failed: ${String(error)}`);
          });
          continue;
        }

        const conversationId = full.from_user_id ?? "";
        const previous = conversationTasks.get(conversationId) ?? Promise.resolve();
        const task = previous
          .catch(() => {})
          .then(run)
          .catch((error) => {
            errLog(`[weixin] message processing failed: ${String(error)}`);
          })
          .finally(() => {
            if (conversationTasks.get(conversationId) === task) {
              conversationTasks.delete(conversationId);
            }
          });
